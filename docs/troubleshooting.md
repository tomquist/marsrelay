...
2. **Power-cycle the battery once Marsrelay is up.** A battery that joined the access point before Marsrelay was running can sit in a stale state for a long time. Boot the ESP32 first, give it a minute, then switch the battery off and on again — do this before you start waiting.
